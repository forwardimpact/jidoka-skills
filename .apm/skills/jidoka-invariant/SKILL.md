---
name: jidoka-invariant
description: >
  Author a declarative invariant rule module for `jidoka invariants`.
  Use when a repository needs to enforce its own architectural rule. Examples
  are a forbidden import, a value that must agree across files, and a directory
  shape. Write it as a `.jidoka/invariants/*.rules.mjs` module the CLI
  discovers and runs.
license: Apache-2.0
metadata:
  version: "0.2.0"
  author: forwardimpact
---

# Write Invariant Rules

`jidoka invariants` is a generic host. It finds every `*.rules.mjs`
under `.jidoka/invariants/`. It runs each module's declarative rules through a
shared engine. It reports the findings. The policy lives in the repository. The
CLI ships only the engine.

Write one module per invariant. A module never imports the engine. It loads
into a consumer repo where the engine package does not resolve. It declares
only policy. The injected kits own all mechanism.

## When to Use

- The repository needs to enforce an architectural rule of its own, such as a
  forbidden import, a value that must agree across files, or a directory shape
- An existing rule module needs a new rule, a scope change, or a migration
  deny-list

## Checklists

<do_confirm_checklist goal="Verify the rule module is sound before committing">

- [ ] State the invariant in one sentence in the module's top comment.
- [ ] Collect subjects in `build` through the kit only. Use no direct `fs`
      access and no subprocess.
- [ ] Make each rule report on violation and return `null` when clean.
- [ ] Add a grandfather `seed` only if the invariant landed on existing
      violations, and keep its deny-list monotone.
- [ ] Confirm `jidoka invariants` fails on a planted violation and passes on
      clean code.

</do_confirm_checklist>

## Process

### Step 1: State the invariant in one sentence

Write the rule as a single claim the code must satisfy ("src files must not
import `node:child_process`", "the version literal must agree across the
manifest and the docs page"). If you cannot state it in one sentence, it is two
invariants. Write two modules. Capture the sentence as the module's top comment.

### Step 2: Choose the subjects

Subjects are the things the rule judges. You group them by **scope**, a label
you pick. Decide what one subject is: a file, a manifest, a matched line, or a
cross-file agreement. Then decide which scope name holds it. A module may
declare several scopes.

### Step 3: Write `build(kit)`

`build` receives the build kit and returns `{ subjects, ctx? }`. Use the kit to
collect subjects. Never reach for `fs` or a subprocess directly. Return one
array per scope, plus optional `ctx` the rules read.

See [references/build-kit.md](references/build-kit.md) for the full kit, and
[references/example.md](references/example.md) for a `build` worked end to end.

### Step 4: Declare the rules

`rules` is a static array, or a `(ruleKit) => array` factory. Each rule names a
`scope`, a `severity`, an optional `when` guard, a `check` that returns a truthy
report on violation (else `null`), and a `message`/`hint`. The build step finds
the candidates. The rules decide pass or fail.

See [references/rule-kit.md](references/rule-kit.md) for the rule shape and the
`parseError`/`failAll` helpers. A worked module is in
[references/example.md](references/example.md).

### Step 5: Add a `seed` only for migrations

When an invariant lands on a codebase with existing violations, grandfather
them. Add an optional `seed(kit)` that prints a deny-list. Store it as
co-located YAML. Have `build` read it through `kit.config`. Each migration PR
removes entries. The list is monotone. Never add to it. Refresh with
`jidoka invariants --seed <module-name>`.

### Step 6: Run it

Drop the module in `.jidoka/invariants/` and run:

```sh
jidoka invariants            # run every module
jidoka invariants --json     # machine output
jidoka invariants --seed <name>   # print a module's seed text
```

Any finding fails the run. Confirm the rule fires on a known violation and
passes on clean code before you commit.

## Documentation

- [Jidoka Instruction Architecture Standard](https://github.com/forwardimpact/monorepo/blob/main/JIDOKA.md)
  — where invariants sit in the layered architecture.
- [Jidoka website](https://www.jidoka.team/)
  — the standard's story: built-in quality, stop the line.
