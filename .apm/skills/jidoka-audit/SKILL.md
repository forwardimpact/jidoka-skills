...
description: >
  ...
  failure, or before a release. Triage every finding and route it to the
  fix that owns it.
license: Apache-2.0
metadata:
  version: "0.2.0"
...
This skill is the maintenance loop for the Jidoka instruction architecture.
`jidoka` runs three checks. This skill turns their findings into fixes. It
routes each finding to the skill that owns the layer. Fix the defect where the
line stopped. Never pass it downstream.
...
- [ ] Route every finding to its owning fix. Do not silence one in place.
- [ ] Regenerate stale job blocks. Do not hand-edit them.
- [ ] Grandfather only through a module's `--seed` path, and only during a
      real migration.
- [ ] Confirm `jidoka` passes with no findings.
...
one-line reason. Group the findings by subcommand before you fix anything. The
fix path differs by kind. A length breach is a different problem from a stale
job block.
...
Run `jidoka` again after each fix. Any finding fails the run. A clean run is
the bar. A re-run also catches the case where one fix exposed another. When you
trim a layer, you can reveal a checklist that now needs its own block.

### Step 4: Record what recurred

If the same class of finding returns again and again, the layer that owns it is
incomplete. Do not blame the contributor. Note the class that returns. Then
strengthen the procedure, reference, or invariant that should prevent it. A
check that fires every week trains people to ignore it.
